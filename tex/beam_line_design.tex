...
\lsnote{You need to summarize what is in this chapter.  Suggested rough list of items.}
...
\lsnote{\sout{As described in Chapters 2, a} A} fast rise time kicker is needed to separate the 
bunch trains supplied by the drive \lsnote{\sout{line} gun.  When the kicker is on, bunches passing though it are deflected into an alternate beamline by the kicker field.} The spacing between bunch trains is on the 
order of nanoseconds\lsnote{; it must be long enough to allow the kicker to reach full field before the arrival of the second bunch train, and also} depends on the layout of the beam lines and laser timing.  \lsnote{The drive beam and the accelerated `witness' beam must be synchronized so that the acceleration field is maximum when the witness beam arrives at the accelerating structure.}
Magnetic options \lsnote{for transverse bunch train separation}, such as a dipole, were disqualified due to their slow rise times 
\lsnote{\sout{(on the order of seconds)} \it{Just leave this out instead of worrying about it}}. \nrnote{is that right? Might be miliseconds? neet to find source for this}
\lsnote{\sout{Therefore an electrostatic approach was chosen, due to achieve faster rise times.}}
...
what angle the beam will be deflected. Longer plates result\lsnote{\sout{s}} in the beam traveling 
in the electromagnetic field for a longer \lsnote{\sout{amount of}} time (\lsnote{\sout{i.e.} yielding a} larger \lsnote{beam deflection} angle). 
A smaller gap \lsnote{increases} the field intensity for a \lsnote{given} voltage \lsnote{\sout{consider} (similar} to a parallel plate 
capacitor), \lsnote{increasing the beam defliection angle. \sout{ and Guass' law. This can also increase the angle. } }


The kicker is essentially a parallel plate waveguide. 
A $\pm$\SI{30}{kV} power supply \lsnote{can \sout{will}} be used to induce a maximum of \SI{60}{kV} potential difference 
across the plates. Each plate will be terminated in a 50 $\Omega$ load to induce a steady 
state current on the plates. \lsnote{\it{Are you sure about that?  I would have thought it was to avoid reflections, which in a sense keeps the current steady, but steady state usually means as opposed to AC current.  Also, what are the subscripts $v$ on the fields for?  Did you mean $E_{x}$ and $B_{y}$}?} The combination of the two plates will result in a static TEM mode 
between the plates. \lsnote{\sout{Calculation of the} The} electric \lsnote{field}, $E_v$, and magnetic field, $B_v$,
...
\begin{figure*}
	...
	\caption{Simplified drawing of the TBA layout. 
			...
			The two beam lines must also be at least \SI{0.5}{m} apart.}
\end{figure*}\label{fig:triangle}
\lsnote{For some reason the figure reference below is not correct in the .pdf file.  Maybe because the figure lable is after the end of the figure.  Also, you should either label the dipole, or better, the dipole bending angle in the figure.}
Using geometry as shown in Figure \ref{fig:triangle}, an upper limit on the angle is based on the existing dipoles at the AWA.
The maximum bend angle is $20^\circ$, meaning the combined kicker and septum angle, must be below this. \lsnote{twenty degrees seems far off the 2 degree target for the kicker angle.  Is that because the septum angle is huge, or did you mean 20 mm?}
That way the bent beam line can be straightened out using a standard dipole, which is available and housed in the AWA bunker. 
It is also considered better to design away from the limit of the equipment. Therefore a bend angle less than $20^\circ$
is preferred and allows room for tolerance errors. 

With these limitations, the septum that will follow the kicker.
...
for the transverse offset of the beam as it exits the kicker. \lsnote{You need your simulations here, with your estimate of the beam size, and give the actual dimensions of the kicker gap to compare.} With the pulsar being variable between 18 and 28 kV,
