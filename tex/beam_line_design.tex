...
A few mechanical constraints were now considered. One beam pipe will connect the kicker and septum.
In this area, the deflected and undeflected beams must travel in the same pipe until reaching the septum.
After the septum, the two beams are separated into their own beam pipes.
The connection piece before the split, is a unique and custom piece designed at 
...
In this regard, a larger angle would help by shortening the pipe length between the kicker and septum.
\begin{table}%[h!]
	\begin{center}
...
This angle would satisfy the one inch pipe spacing in the septum as well. 
...
Using the x offest numbers found in Figure~\ref{fig:linear}, 
we can compare the measured angle vs. first order approximations from Section~\ref{theory}.
\nrnote{Need to know exact location of YAG screen to finish this analysis.
	Drift between kicker plate and YAG screen is probably adding xoffset to beam.
I will go to AWA to measure this, and plot centroid calc on Fig 2.11 afterwards. }
\begin{figure}
	\centering
...
\begin{figure}
	...
	\caption{Beam size data during voltage scan. 
		Higher voltage on the kicker resulted in larger x offset of the beam centroid.}
\end{figure}




\lsnote{Yes - a comparison of the beam sizes to simulation, and a calculation of the deflection verus predicted deflection.  Also, there is a discrepancy, in figure 4.2 the beam size at kicker exit is smaller, not larger.}
...
\nrnote{Notes from call: short discussion of the issues, what are you trying to achieve, where are apetures tight.
