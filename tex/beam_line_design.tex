...
Independent staging requires two \lsnote{drive} beam lines, with routing of bunch trains via a kicker and septum, 
...
This second independent beam line \lsnote{design has been completed \sout{was designed}}. So far, installation and testing of the kicker has been done.  
...
\caption{The arrows indicate what direction the beams travels.
	...
	The witness line has one accelerating cavity with a \lsnote{maximum} beam energy of \SI{15}{MeV}.}
...
\lsnote{\sout{easily or}} readily available equipment, and mechanical constraints in the AWA tunnel. 
...
in the electromagnetic field for a longer time yielding a larger beam deflection angle\lsnote{{\it extra parenthesis}\sout{)}}. 
...
\lsnote{{\it Above you said max beam energy is 65 MeV, and below 75 MeV, not consistent, so explain.}}
...
\lsnote{\sout{A}} Kicker lengths of \SI{0.42}{m}, \SI{0.5}{m}, and \SI{0.6}{m} were investigated.
...
AWA \lsnote{by Scott Doran}, and ordered from MDC Vacuum Products\lsnote{\sout{, by Scott Doran}}. 
...
\lsnote{{\it There appears to be an error in the table, you have the beam energy? instead of kicker plate voltage}}
...
angle and x offset required in the beam line. \lsnote{unclear, do you mean you were seeing if a low energy beam would make it through as well?}
...
\begin{figure*}
	...
	Power is suppled by a \lsnote{pulsar \sout{puslar}} also fabricated by FID GmbH and borrowed from the APS Electronics group.}
...
and quadrupoles 5, 6, and 7 were either \SI{-1.53}{A} \lsnote{or \sout{and}} \SI{3.08}{A} in a nearly 1:2:1 configuration.
All accelerating cavities were operated at \SI{-20}{^\circ} off crest to minimize energy spread.
The centroid \lsnote{position} of the beam was \lsnote{measured \sout{calculated}} for several kicker voltages, and a linear fit was performed.
...
we can compare the measured angle vs. first order approximations from Section~\ref{theory}.  \lsnote{How did you get the angle from the data?  Meaning, what were the positions of the YAGs that you used?  I assume it was a sort of tan(theta)=yag offset/long distance kind of thing?}
\begin{figure}
	\centering
	\includegraphics[width=0.75\textwidth]{./images/kicker_angle_comparison}
	\caption{Measured angle provided by the kicker at several voltages.  \lsnote{{\it What was the beam energy during the measurements?  Also, the units should have been degrees?}}
...
the large discrepancy \lsnote{in the horizontal and vertical beam size} can not be attributed to the kicker alone.
...
\begin{figure}
	\centering
	\includegraphics[width=0.8\textwidth]{./images/xybeamsizes_high_charge_kicker_scan_angle}
...
\lsnote{{\it In the figure, it looks like the simulation also predicts that the beam is not round.  So, the source of the asymmetry must be known?}}
There are several possible sources for the asymmetries between the x and y dimensions 
in Figure~\ref{fig:beamsize}. These include the initial laser profile (which was not exactly symmetric
when the data was taken), the off axis fields in the accelerating cavities, 
and uncertainties in the quadrupoles strengths.
If the asymmetries were due to the kicker, they would not be present when the kicker is off.
Persistence of the asymmetry when the kicker is off indicates 
the sources of error can be fixed with better control of the quadrupoles, 
and is not an intrinsic problem associated with the kicker.


\Section{TBA Beam Line}

\lsnote{While \sout{With}} the kicker and septum parameters \lsnote{were} defined by the constraints dicussed above, 
the optics \lsnote{must be used to address the \sout{and}} aperture limits further down stream \lsnote{\sout{were considered}}.
...
it remained to determine if an optics solution existed to satisfy the 
...
For transmission, an aperture of \SI{17.6}{mm} \lsnote{exists} at the PETS.
This is roughly three times smaller than the rest of the beam line.

As a first test, a point to point matrix calculation was done 
to determine if the beam size could be maintained near the kicker.
Next the information in Chapter~\ref{simulations} was used to form the 
...
\lsnote{The effect of a beamline component (such as a dipole, quadrupole, or straight beam pipe) on the phase space coordinates of a particle can be modeled as a matrix operator}~\cite{brown,Wiedemann}.  \lsnote{These building blocks may be `stacked' to model the effect of an entire beamline, or section of a beamline, on the coordinates of a  particle.  \sout{From many sources~cite(brown,Wiedemann) the building blocks of any 
beam line can be defined. These typically include drift, quadrupoles, and dipoles.}}
\lsnote{These \sout{Using these}} common building blocks \lsnote{were used as} a check \lsnote{that \sout{of}} the 
\lsnote{beam properties \sout{ transport matrix for} } after the 
first four quadrupoles \lsnote{would allow transport through the kicker \sout{was done,}} (see Figure~\ref{fig:quads}),
\iffalse
\begin{figure}
	\centering
	\includegraphics[width=0.75\textwidth]{./images/p2p_optics}
	\caption{\nrnote{I will add focal length label to this plot.}}
	\label{fig:p2p}
\end{figure}
\fi
\lsnote{by \sout{This established} establishing that} the beam size could be maintained near the kicker during transport.  
...
the quadruplet telescope module as described by K. Brown in \cite{brown} is used.  
...
for $f_1$.  \lsnote{{\it How does this give the value of f1?}}
...
for the focal length $f_1$: \lsnote{{\it Are you missing an assumption?  The following eq. says to me that you are assuming that the center of  a quad is at the focal point of a previous quad..}}
\begin{align}
...
max strength is \SI{9}{[T/m]}. This also gives some indication that 
that there should be several possible solutions found during optimization
since the point to point check is not at either extreme of the quadrupole limits.L 
...
These two conditions combined will provide an optics \lsnote{solution \sout{solutions}} that provides the most power output from the \lsnote{PETS structure for acceleration of the witness beam \sout{test}}. 
