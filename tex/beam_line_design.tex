...
If the beam is bent at a smaller angle, less coherent synchrotron radiation (CSR) and energy spread will be induced.\lsnote{{\it Did you mean transverse position spread?  Magnets cannot increase energy spread.  Unless the CSR does that - but - I don't think that likely.}}
...
Around this time, it was also learned that the \lsnote{maximum \sout{max}} pulsar \lsnote{voltage} was $\pm$ \SI{30}{kV}, not $\pm$ \SI{35}{kV} as expected. 
...
which could complicate installation if the chamber \lsnote{were \sout{was}} too long. 
...
A new table of voltages and angles \lsnote{were \sout{was}} calculated. At this plate length, 
...
With the pulsar \lsnote{output} being variable between 18 and 30 kV,
several feasible options were calculated using the equations in Section \ref{theory}, 
and are shown in Figures~\ref{fig:kickerangles} and \ref{fig:kickeroffset}\lsnote{.}
\begin{figure}%[h]
	\begin{center}
...
A simplified experimental setup is shown in Figure~\ref{fig:kyag}\lsnote{.}
\begin{figure*}
	\begin{center}		
		\begin{tikzpicture}[scale=0.7]
		\node (fig1) at (0,0)
...
The \lsnote{latter \sout{later}} part can be approximated \lsnote{using the \sout{as a}} triangle \lsnote{\sout{as}} shown in Figure~\ref{fig:kicker-geometry}.
...
\begin{equation}
	\Delta x = \SI{0.78}{m}\,\,\theta_{k}
		\label{eq:kdx}
\end{equation}
\lsnote{The position offset on the YAG screen due to the kicker \sout{In Equation~\ref{eq:kdx}}}, $\Delta x$, was measured and \lsnote{\sout{is}} the data \lsnote{is} shown in Figure~\ref{fig:linear}. 
...
\end{figure}
The kicker slightly \lsnote{increases \sout{increase}} the beam size in the kicked direction (x), 
...
the \lsnote{source \sout{sources}} of error is not an intrinsic problem associated with the kicker.
\begin{figure}
	\centering
...
There are several possible sources of the asymmetric beam \lsnote{size \sout{sizes}}. 
...
To check the laser radius contribution, \lsnote{simulations with} symmetric and asymmetric \lsnote{laser cross-sections \sout{simulations}} were done.
...
are strongly affecting the results \lsnote{presented \sout{seen}} in Figure~\ref{fig:beamsize}.
...
\lsnote{due to a distribution \sout{distributed in} of} lower gradients \lsnote{in \sout{for}} the cavities. 
...
While  the \lsnote{kicker \sout{er}} and septum parameters were defined by the constraints discussed above, 
...
With the third item \lsnote{\sout{addressed}} partially addressed \lsnote{\sout{in} (see Section~\ref{theory})}, 
...
that a shorter bunch will generate more power in the PETS structures [ref here]. \lsnote{{\it <-Put in a reference or remove} Since the PETS 
\sout{Regarding transmission, an}} aperture of \SI{17.6}{mm} \lsnote{\sout{exists at the PETS.
This}} is roughly three times smaller than the rest of the beam line, \lsnote{care must be taken to achieve  good transmission there.}

As a first check \lsnote{of the rough design}, a point to point matrix calculation was done 
to determine if the beam size could be maintained near the kicker.
Next the \lsnote{simulation studies \sout{information}} presented in Chapter~\ref{simulations} \lsnote{were \sout{was}} used \lsnote{as a \sout{to form the}} 
base line \lsnote{for \sout{of} the final \sout{further}} optimization. \lsnote{\sout{efforts.}} The genetic algorithm in OPAL
was used to \lsnote{achieve \sout{explore and optimize}} the optics design \lsnote{required for \sout{in the} testing fully staged TBA. \sout{beam line.}}
Solutions were found that meet the requirements listed above.



\Subsection{Point to Point Check}
The effect of a beam line component (such as a dipole, quadrupole, or straight beam pipe) on the phase space coordinates of a particle can be modeled as a matrix operator~\cite{brown,Wiedemann}.  These building blocks may be `stacked' to \lsnote{determine \sout{model}} the effect of an entire beam line, or section of a beam line, on the coordinates of a  particle.  
...
The transfer matrix R, \lsnote{is} defined as:  
\begin{equation}
...
\frac{1}{f} = kl 
\end{equation}
Where $l$ is the quadrupole's effective length, and k is the gradient \lsnote{with respect to \sout{w.r.t}} 
...
\end{equation}
The beam energy \lsnote{and the effective length of the quadrupole are needed for a numerical estimate of the quadrupole gradient needed to maintain the beam size at the kicker. \sout{must also be accounted for with the dimensions of the quadrupole.}} 
...
exist at \SI{40}{nC} \lsnote{that allow \sout{where}} the beam \lsnote{to be \sout{is}} transmitted \lsnote{at} 100\% through the PETS structure in the deflected beam line.
...
The\lsnote{built-in \sout{built in}} GA in OPAL was again used for these simulations. 
...
Some \lsnote{drawbacks \sout{draw backs}} of using a GA, \lsnote{are \sout{is}} that many simulation parameters 
...
beam size in \lsnote{\sout{the}} y. This constraint was added to avoid this error.
...
Constraints were evaluated at the entrance of the kicker \lsnote{at} \SI{16.5}{m}, and
...
\begin{table}%[h!]
	\begin{center}
		...
		All six objectives were minimized simultaneously during the GA runs at \SI{19.5}{m}\lsnote{,\sout{.}}
	(entrance of the fifth quadrupole)\lsnote{.}}
	...
	\end{center}
\end{table}
\lsnote{That is the location of \sout{In other words,}} the first quadrupole between the septum and kicker, see Figure~\ref{fig:full-staging}.
Optimizing at this location helps reduce dispersion by minimizing 
the energy spread before \lsnote{the beam \sout{it}} enters the $15^\circ$ bend that straightens out 
...
Details of how \lsnote{to specify} the magnet settings, phases, and element locations defined 
...
chooses the best points \lsnote{with respect to \sout{w.r.t}} the objectives chosen. This guarantees no 
...
The results \lsnote{shown} in Figure~\ref{fig:optresults} are promising, they indicate the optics 
...
The best scenario, corresponding to a matching \lsnote{solenoid} strength of \SI{197}{A}, was then explored further.
...
extreme limits of the \lsnote{magnets \sout{magnetic}}, meaning there is plenty of wiggle room to adjust 
...
increase the time to simulation. \lsnote{These effects \sout{This}} would lengthen the amount of time 
needed to run the GA to over 4 or 5 days.
Therefore, a 2D study was done first, and the solution checked.
\lsnote{Then another \sout{A second}} simulation \lsnote{using \sout{of}} the same design variables was done \lsnote{that included \sout{including}} 
...
\begin{figure}
	...
	\caption{Simulation of the optimized settings in Table~\ref{tab:designopt} with CSR and 3D field maps \lsnote{included}.
...
\lsnote{however, the 2D solution indicated that \sout{and given the solution in 2D,}} there is plenty of range on the solenoid and quadrupoles to do this.
A scan of the matching solenoid was done to \lsnote{\sout{try to}} reduce the over focusing in the horizontal direction\lsnote{(}
see the updated 3D beam line simulation in Figure~\ref{fig:adjsol} \lsnote{)}.
\begin{figure}
	...
	\caption{Simulation of full beam line using optimized quad \lsnote{settings \sout{setting}}, 
...
They also \lsnote{aided \sout{added}} in performing a Time Domain Reflectometer (TDR) measurement to characterize 
