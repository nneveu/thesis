...
The largest drive beam energy achievable at the AWA is 75 MeV. 
...
beam line without competing for transverse space. Using geometry as shown in Figure \ref{fig:triangle}.
\begin{figure*}
		\begin{center}		
		...
		\end{tikzpicture}
		\end{center} 
	\caption{The angle provided by the kicker, $\theta_k$, plus the angle provided by the septum, $\theta_s$,
...
An upper limit on the angle in Figure \ref{fig:triangle}, is based on the existing dipoles at the AWA.
The maximum bend angle is $20^\circ$, meaning the combined kicker and septum angle, must be below this.
That way the bent beam line can be straightened out using a standard AWA dipole, which is already housed in the 
AWA bunker. 

With these limitations we can consider the septum that will follow the kicker.
