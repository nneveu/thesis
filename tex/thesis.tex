...
Throughout the design process, optimization was used to 
shape the beam entering the TBA experimental area, 
minimize the emittance after the six cavity linac, on the drive side, 
and choose the optimal optics configuration for maximum beam transport.
The optimization techniques used in include model based, genetic algorithms, 
...
the beam dynamics in two cases of interest at AWA. 
...
The parameters $v^k$ are generated and supplied to the open source 
particle-in-cell code OPAL\=/T~\cite{opal}. 
...
\Subsection{Linac Simulations Using BOBYQA} \label{sec:pareto}
