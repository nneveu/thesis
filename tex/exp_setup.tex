...
\nrnote{\it We used different methods (he measured forward and rev power from klystron diodes).
My numbers are still smaller than his...still looking for what's wrong here. }
\begin{equation}
...
\SI{53.2}{dB} + \SI{36}{dB} + \SI{0.56}{dB} \\
 + \SI{9.5}{dB}+ \SI{10}{dB} + \SI{14.18}{dBm} \\
 = \SI{95.08}{dBm} 
\end{aligned}
\end{equation*}
...
\end{equation}
\begin{equation} 
P_2 = 10^{\SI{9.508}{dBm}} \cdot  \SI{1}{[mW]} = 3.6 \, \SI{}{[MW]} 
\end{equation}
...
\end{equation}
\begin{equation}
	E_z = 1.86\sqrt{\frac{P_{measured}}{7.498\times10^-3}} = \SI{40}{MV/m}
\end{equation}
Using the measurements above, this gives an estimate for the gradient in the gun.
\nrnote{These are still too low...}
...
\nrnote{AWA is missing those values, I added a sentence about this}
\begin{table} %or [hbt] ?
...
Four and six are the only cavities we can calculate the gradient for, 
but the measurements still give us an idea of which cavities are receiving more power.
...
There are several beam diagnostics that have and will continue to be used in 
TBA experiments at the AWA. They include beam size, energy, bunch length, 
and emittance measurements. The first three will be discussed in detail 
...
is calculated based on the measurements shown in Table \ref{tab:counts}.  
\begin{table}
	\centering
...
using Equation~\ref{eq:xoff}. This result is confirmed by a simulation in OPAL. 

Now consider a second more realistic experimental case.
The energy is unknown, but the current supplied to the magnet is given as \SI{0}{A}.
Using Equation~\ref{eq:b}, the magnetic field is calculated, \SI{0}{T}.
\nrnote{finish this thought with real numbers...}
Given a ......  energy of $\SI{64.8}{[MeV]}$ given measured counts of 
