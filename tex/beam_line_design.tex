...
\nrnote{mismatch in energy was a typo}
...
and quadrupoles 5, 6, and 7 were either \SI{-1.53}{A} or \SI{3.08}{A} in a nearly 1:2:1 configuration.
...
\nrnote{rewrote this section to explain angle calculation}
...
Inside the kicker, the trajectory is approximated using Equation~\ref{eq:}. 
Combining these two, the equation for the x offset seen during the experiment
can be written as:
\begin{equation}
...
\end{equation}\label{eq:kdata}
Where $\rho$ is the bending radius in the kicker, and $L_{drift}$ is \SI{0.3}{m} as shown in Figure~\ref{fig:kyag}.
Using the small angle approximation, this equation simplifies:
\begin{equation}
...
\end{equation}
In Equation~\ref{eq:kdata}, $\Delta x$, was measured and is the data shown in Figure~\ref{fig:linear}. Note, the beam centroid location at kicker voltage $=0$, 
...
(i.e. $\Delta x_1 = x_{v1} - x_{v0}$)

With these $\Delta x$ values, Equation~\ref{eq:kdata} can be rewritten to 
solve for the experimentally achieved angle:
