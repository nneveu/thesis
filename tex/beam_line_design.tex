...
\begin{figure*}
	\begin{center}		
		...
		\node[fill=white, inner sep=2pt] (txt2) at (2.3,3.8) {$\theta_k$};
		\end{tikzpicture}
	\end{center} 
...
\begin{figure}
	\begin{center}		
		\begin{tikzpicture}[scale=0.7]
		\node (fig1) at (0,0)
		{\includegraphics[width=0.48\textwidth]{./images/scatter_kicker_entrance}%
		\includegraphics[width=0.48\textwidth]{./images/scatter_kicker_exit}};
		\node[fill=white, inner sep=2pt] (txt2) at (6.2,-3.8) {X [mm]};
		\node[fill=white, inner sep=2pt] (txt2) at (-5,-3.8) {X [mm]};
		\end{tikzpicture}
	\end{center} 
...
\end{equation}
where $f_1 \ldots f_4$ stand for the focal lengths of each quadrupole before the kicker. 
...
quadrupoles in the beam line, \SI{3.8}{m}, and referring to Figure~\ref{fig:p2p}, 
...
Given an energy of \SI{65}{MeV}, and a quadrupole length of \SI{11}{cm}, 
this configuration would require a 
...
The built-in GA in \verb|OPAL| was used for these simulations. 
...
and there is no clear guidance on what type of problem setup will be 
...
3D beam size, transverse momentum, and energy spread at the fifth quadrupole on the drive line.
...
may not have the same input parameter settings for the matching solenoid or quadrupole strengths, as the smallest x position 
...
design variables and beam sizes for Figure~\ref{fig:tba-pareto}.  
\begin{table}%[h!]
	\begin{center}
		\caption{Parameters for large scale TBA optimization runs.
		...
		\begin{tabular}{lcccc}
			...
			Quadrupole 1 [T$\cdot$m]		& -0.8	&  1.10  & -0.72 & -0.21 \\ 
			Quadrupole 2 [T$\cdot$m]		& 0.9	&  -1.88 & 0.20  & 0.37  \\
			Quadrupole 3 [T$\cdot$m]		& 0.8	&  0.39  & -0.73 & -0.26\\
			Quadrupole 4 [T$\cdot$m]		& -1.0	&  0.61  & 1.92  & 0.25 \\ 
