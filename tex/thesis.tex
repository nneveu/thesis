...
\nrnote{Copied and pasted benchmark here, need to clean up}
...
Since no members of AWA had experience with OPAL-T, this benchmark 
was done to compare results to GPT and ASTRA in two of the three 
main collective effects of interest to the AWA: SC and CSR, 
with wakefields being the other effect of interest. 

\nrnote{this paragraph might need to move or be taken out}
...
The rf gun and solenoid fields seen by the beam are shown in Fig. 1. 
Note, in the remainder of this paper, the word gun is used 
...
target without any optimization. A coarse 1D minimization of the 
emittance was done to determine the value of the laser radius 
used in this benchmark. The resulting minimum emittance was   
\SI{1.16}{\micro\metre}. A genetic and multi-objective optimizer
was not used in any of the codes. A multiobjective genetic optimizer  
may have been able to achieve a lower minimum emittance by varying
the laser radius and matching solenoid simultaneously. \nrnote{this was done later w/ Jeff}
The resulting minimum emittance was \SI{1.16}{\micro\metre}. 
...
the benchmark are listed in Table 2. The rf gun and solenoid
field maps were generated with the SUPERFISH/POISSON codes [13].
The gradient was chosen to match typical operations at PITZ [11] 
and the AWA. Note that the codes use various methods to model 
...
transverse components using the paraxial approximation (e.g.). 
The simulations used a 2D cylindrical-symmetric SC algorithm with a uniform  
particle-deposition mesh; see ASTRA user manual pg. 8 \cite{astra}.
The radial and longitudinal number of cells composing the mesh 
were taken to respectively  with  100k  particles.  
The image charge close to the cathode was accounted for until the bunch reached 
9.7-cm from the cathode surface. 
GPT read in the 2D electric and magnetic field files,  
and used a square 3D adaptive SC mesh of 
with 100k particles, see spacecharge3Dmesh option in GPT manual pg. 132 \cite{gpt}.
...
structured equidistant SC mesh, see OPAL manual pg. 27 for SC calculation \cite{opal}.  

Several square mesh sizes were run, the results plotted in 
Fig. 2 and 3 correspond to a mesh of 46 with 1 million particles.  
The image charge calculation uses a shifted integrated Green function \cite{imagecharge}.  
...
See Figs. 2 and 3 for beam envelopes in the gun and drift. 
...
are shown in Table 3. The CSR routine used in OPAL-T is based on 
...
\Appendix{How to build OPAL on Bebop}
