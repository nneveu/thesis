...
During the beam tests the accelerator was operated in TBA-like conditions \lsnote{with machine settings} based on simulations 
...
The R value for the fit in Figure \ref{fig:linear} is 0.99704. This \lsnote{shows \sout{suggests}} that the beam offset 
...
\lsnote{\sout{we can compare}} the measured angle vs. first order approximations from Section~\ref{theory} \lsnote{can be compared {\it passive voice}}. 
Note the x offset seen on the YAG screen is not solely due to the trajectory in the kicker.
The \lsnote{contributions to the total deflection \sout{set up}} must be broken into two parts: \lsnote{\sout{when}} the \lsnote{deflection of the} beam \lsnote{\sout{is}} inside the kicker, 
and \lsnote{the position change} when \lsnote{the beam \sout{it}} is drifting towards the YAG screen after it exits the kicker.
...
\begin{figure}
	...
	\caption{Beam size data taken during kicker voltage scan. Asymmetry between the x and y beam sizes
         ...
         The \lsnote{gun} gradient \lsnote{to get the simulation values} shown in this figure \lsnote{was \sout{is}} \SI{55}{MV/m}. \lsnote{\sout{Further scanning and to improve agreement
...
are strongly \lsnote{affecting \sout{effecting}} the results seen in Figure~\ref{fig:beamsize}.
...
\lsnote{This \sout{Which}} is considerably lower than the reported AWA maximum gradient of \SI{85}{MV/m}.
...
Note, these errors are not a major hurdle for TBA experiments, as they \lsnote{can}
...
it remained to determine if an optics solution \lsnote{exists \sout{existed} that can \sout{to}} satisfy the 
...
\lsnote{Regarding \sout{For}} transmission, an aperture of \SI{17.6}{mm} exists at the PETS.
This is roughly three times smaller than the rest of the beam line.

As a first \lsnote{check \sout{test}}, a point to point matrix calculation was done 
to determine if the beam size could be maintained near the kicker.
Next the information \lsnote{presented} in Chapter~\ref{simulations} was used to form the 
...
Four quadruple strengths and the six \lsnote{{\it Isn't it 5?}} distances between the quadrupoles are parameters under consideration. 
To reduce the number of variables from ten to four, \lsnote{the drift lengths are set to be sums of focal lengths, as in}
the quadruplet telescope module described by K. Brown in \cite{brown} \lsnote{\sout{is used}}.  
The transfer matrix R, defined as:  \lsnote{You have only 3 quads below}
\begin{equation}
...
\lsnote{some of} which include:
\begin{itemize}
	\item using too many objectives (6+)
	\item including too many design variables (15+)
	\item over constraint of the objective values \lsnote{\sout{output from the simulation}} (upper bound too small at \SI{0.1}{mm} for beam sizes)
	\item using bad hyper parameters
	\item \lsnote{setting \sout{Setting}} the first optimization point upstream near the linac
\end{itemize}
\lsnote{\sout{and so on.}} 
...
\lsnote{{\it Minimizing the energy spread would minimize the increase in beam size due to dispersion - is that what you meant?  Minimizing the beam size does not affect spread due to dispersion.}}
\begin{table}%[h!]
	\begin{center}
...
\end{figure}
Remember, up to now, the optimization and and simulations have taken place using
