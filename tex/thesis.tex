...
Then a signal was sent to the control room where it was 
where the cavity normally is. Then a signal was propagated to the control room where it was 
...
The power reading in the control room was $P_{cr} = \SI{-9.48}{dBm}$, and the power reading in 
the tunnel was $P_t = \SI{-8.92}{dBm}$. The cables and $\SI{10}{dB}$ attenuator connected 
to the control are equivalent to an attenuation of $P_{o}\approx\SI{0.56}{dBm}$. 
However, one more cable needs to be accounted for and 
subtracted from the overall attenuation. A short blue heliax was used to connect the 
signal generator to the control room cables, as shown in Fig.~\ref{fig:tikzcalibration}.
This cable is not normally connected to the control room as shown in Fig.~\ref{fig:tikzdrivegun}.
>>>>>>> Stashed changes
...
Using the set up in Fig.~\ref{fig:tikzdrivegun}, the power meter measured  $P_{3} \approx \SI{-8.66}{dBm}$
...
we can add $P_o\approx\SI{0.56}{dBm}$ to the raw power meter reading, which gives $P_2\approx\SI{-8.1}{dBm}$.
...
With the power in units of mW, we can infer the strength of the original signal
by using the relationship between power and attenuation \cite{pozar}. 
power before the \SI{36}{dB} attenuator, $P_1$:
\begin{equation}
\SI{36}{[dB]} = 10 \cdot \log{\frac{P_{1}}{P_{2}}}
\end{equation}
\begin{equation}
P_1 = 10^{3.6} \cdot P_2 = \SI{617}{mW}
\end{equation}

Somethings wrong here (above)....
