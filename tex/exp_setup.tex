...
\begin{figure}
	...
	\caption{AWA Facility bunker and simplified TBA staging layout. 
		...
		The witness beam line (left) supplies \lsnote{a} low charge \lsnote{single \sout{one}} bunch at \SI{15}{MeV}. 
...
\begin{figure}
	...
	\caption{Simplified drawing of the two staging schemes.
		...
		through and loses energy in the first stage before reaching the second stage \lsnote{(top subfigure)}.
		This is prevented by separate beam lines in the full staging layout \lsnote{(bottom subfigure)}. }
...
A simplified diagram of \lsnote{the laser pulse propagtion \sout{this is timing}} is shown in Figure~\ref{fig:lasertiming}, 
...
\begin{figure}
	...
	The spacing in the pluse train is one period of the \lsnote{sinusoidal} RF \lsnote{voltage} supplied to the cavities,
	which is \SI{1.3}{GHz} \lsnote{\sout{or} (T=}\SI{769}{ps}\lsnote{)}. }
	%\label{inter}
	%\caption{Bolometer. }
...
simplify the experimental setup, and \lsnote{to reduce \sout{reduced}} interruption to beam time 
...
\lsnote{so} a lower \lsnote{pulse intensity \sout{value}} is expected in experimental measurments. The path of laser pulse one and
pulse four in the train can be described \lsnote{in the format \sout{by an updated version}} of Equation~\ref{eq:i4}.
The number of splitters is \lsnote{\sout{reduced to}} four, and both pulses are transmitted three 
...
increased uniformity in the bunch train furthers the goals of \lsnote{demonstrating} TBA 
...
In order to accurately simulate the RF fields in the \lsnote{linac} cavities \lsnote{and gun} on the drive and witness line, 
...
power splitters and attenuators. If the power is \lsnote{not} split \lsnote{evenly \sout{exactly}}, the ratio 
...
also be \lsnote{a \sout{the}} source of power difference at the cavities.
 
In an effort to \lsnote{determine \sout{understand}} these differences, 
...
\lsnote{\sout{taken}} are \lsnote{accurate \sout{not misleading}}. The cable effects can be taken into account
...
\begin{figure*}%[h]
	\begin{center}	
		...
		\end{circuitikz}
	\end{center} 
	\caption{Experimental setup when calibrating \lsnote{\sout{the}} cables from \lsnote{the} gun \lsnote{location} to \lsnote{the} control room. 
...
\lsnote{A power measurement was done to determine the accelerating gradient in the gun.}
...
The radius of curvature, $\rho$, can be calculated using the \lsnote{geometry shown in the figure, \sout{geometric equation for sin,}} and 
...
The dipole length and \lsnote{bending} angle are \lsnote{\sout{defined as,}} $L=\SI{0.2}{m}$, \lsnote{and \sout{angle,}} $\theta=\SI{20}{^\circ}$. 
...
using Equation~\ref{eq:xoff}. This result was confirmed by a simulation in OPAL.   \lsnote{{\it The previous two equation references are broken.}}
...
\begin{figure}
	...
	\label{interferogram}
\end{figure}
To alleviate the effect of charge fluctuations, \lsnote{\sout{we recorded}} 15 bolometer values for each data point \lsnote{were recorded}.
...
the gain set to 200.
For the case of 1 nC \lsnote{electron \sout{electrons}} beams, the laser transverse profile was homogenized prior to the vacuum injection \cite{PhysRevAccelBeams.20.103404}.
...
A significant amount of \lsnote{preparation \sout{experimental investigation}} is 
required before the demonstration of fully staged TBA.
\lsnote{\sout{Work in these areas has begun and continues.}} 
\lsnote{Since uniform bunch trains are desirable, the \sout{The}} UV laser pulse train intensities were measured and significantly improved.
It was discovered that the UV splitters were contributing to non-uniformity in the laser intensities. 
\lsnote{A 16.5 percent improvement laser pulse uniformity was achieved \sout{in the This was mitigated}} through the purchase and characterization of 
new splitters with a tighter tolerance on the UV coating. 
The best performing splitters were chosen \lsnote{, sorted} and installed at the AWA, 
where they are currently \lsnote{in use. \sout{used}}  

\lsnote{The power level (and associated fields) in the gun and linac structures must be well understood so that simulations can accurately guide optimization of the machine.}
RF power measurments using the \lsnote{pickup \sout{pick up}} probe in the gun were made and compared to previous 
measurmeents using the waveguide couplers. 
Measurments were also made for the linac tanks, \lsnote{although \sout{but}} there exist no alternative data set to compare to at the present time \lsnote{, it was still possible to gain an understanding of the relative power levels in the linac cavities.  The power balance has an impact on beam dynamics}.

\lsnote{Finally, the beam diagnostics and data processing must be understood, to make comparisons of experimental parameters to simulation results, and to quantify the success of fully staged two beam acceleration.}  The procedure for energy measurments using a spectrometer was described, \lsnote{along with an example measurement, since beam energy is a critical parameter of two beam acceleration. \sout{An example energy measurment was given.}}
Beam image analysis techniques were discussed in detail.
\lsnote{\sout{All the code used is freely available on github.}}
This includes fiducial calculation, charge selection, background subtraction, 
calculation of beam sizes, and plotting proflie distributions.  \lsnote{All the code used is freely available on github.}
An experimental measurement \lsnote{that used \sout{using}} the image anaylsis techniques \lsnote{was presented \sout{is shown}}. 
The experiment was a solenoid scan at the first YAG screen on the drive line.
The results informed the AWA \lsnote{group} that the gun gradient is lower than expected.
