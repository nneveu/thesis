...
\lsnote{{\it new paragraph}} A kicker design from Indiana University (IU) was used as a base line to design a kicker specific to the
...
Two \lsnote{\sout{out of}} vacuum tests were performed to verify the fabrication process was successful.
The electronics group at the Advanced Photon Source (APS) provided equipment and \lsnote{helped perform} these tests.
...
\lsnote{\sout{A HV pulsar was borrowed from APS for the tests.}} 
\lsnote{The \sout{We found the} kick was found} to be linear with respect to \lsnote{the} voltage of the pulser.  The angle was slightly lower than \lsnote{the} simulation \lsnote{result\sout{efforts}}, probably due to reflections at the
...
where it was also fabricated. \lsnote{\sout{The septum was shipped to ANL and arrived in August.}} 
...
\lsnote{A \sout{At first}} point to point matrix calculation was done to ensure that transport of the
beam was possible through the desired kicker gap.  \lsnote{This was followed by \sout{
Then}} an optimization \lsnote{\sout{was}} performed \lsnote{using the OPAL code and a \sout{in using the} built-in genetic algorithm \sout{GA in OPAL}.} 
\nrnote{assuming GA acronym will be defined in simulation chapter?}\lsnote{yes, fine, but write out GA here}
\lsnote{There were \sout{The}} optimization results in several optics scenarios that can transport 
the beam through the PETS with 100\% transmission.  \lsnote{\sout{
...
\lsnote{This chapter will start with a discussion of the design considerations for the kicker modification for the AWA facility.  This is followed by a description of the hardware tests done before the kicker installation.  Simulations relevent to the kicker design will be presented, and compared to beam measurement results.  The chosen optics design based on the optimization studies will then be presented.}
...
\lsnote{where \sout{Where}} h is the gap height. From Maxwell's equations, we know that the electric and magnetic 
...
Since the force due to the electric field is equal and oriented in the same direction as the \lsnote{force \sout{field}} due to the magnetic field, 
...
\lsnote{\sout{a}} tables \lsnote{were made for different plate lengths giving \sout{of}} possible gap, angle, and x offset values \lsnote{\sout{were made}} for comparison. 
The \lsnote{maximum \sout{max}} beam energy was used, \SI{75}{MeV}, \lsnote{and} the expected pulsar voltage was used, \SI{35}{kV}.
...
\begin{figure}
	...
	\caption{\lsnote{Simulation of the \sout{Beam}} size of a \SI{40}{nC} beam at the entrance and exit of proposed kicker \lsnote{configuration}.}
...
\lsnote{the units are wrong in your figure, m instead of mm} 
This requires that the kicker gap \lsnote{be comfortably \sout{is}} larger than \SI{20}{mm}, 
\lsnote{and have \sout{plus}} room for the beam centroid deflection that the kicker will cause.
...
Around this time, it was also learned that the max pulsar was $\pm$ \SI{30}{kV}, not $\pm$ \SI{35}{kV} as \lsnote{expected. \sout{previously thought.}}
These two factors would drive the possible kick angle below \SI{2}{^\circ}, if the kicker plates remained the same length.
The next question was whether \lsnote{{\it try to stick to passive voice} \sout{we should} to} push for a larger angle by elongating the kicker plates, 
or \lsnote{whether \sout{if the current}} values at \SI{40}{mm} and above were sufficient.
\lsnote{Did you mean 40 cm?}
A few mechanical constraints were now considered. 
\lsnote{Since the deflected and undeflected beams \sout{bent beam}} must travel in the same pipe \lsnote{\sout{as the straight through}} until reaching the septum, 
...
a PIC \nrnote{(assuming PIC is defined in simulations cp?)} \lsnote{yes, fine} simulation was done to estimate the beam 
...
\begin{figure}
	...
	\caption{Simulation of the transverse offset of a \SI{65}{MeV} beam as 
		...
		is nearly \SI{20}{mm} as predicted in Figure \ref{fig:kickeroffset}.
	       The kicker is now at \SI{16.5}{m} due to other experiments at the AWA.
\nrnote{not sure if I should address this} \lsnote{just leave last sentence out}}
	\label{fig:beamtraj}
\end{figure}
\nrnote{Cutting CST all together because they are only impedance calculations}\lsnote{yes, fine}
...
Ideally, the response would be \SI{50}{\ohm} to reduce \lsnote{\sout{the amount of}} reflections and resulting power loss. 
...
\begin{figure*}
	...
	\end{subfigure}
	\caption{TDR measurements \lsnote{of the impedance} of AWA and APS kickers, plotted by C. Y. Yao. 
	Single mode refers to the orientation of the measurement. Only one port of the kicker is driven and measured at a time.}
	\label{fig:TDR}
\end{figure*}
...
\lsnote{\sout{Next the} The kicker was installed in the AWA drive beam line \sout{.
It's approximate location is} approximately} 16.5 meters downstream of the cathode.
The HV pulsar was located on the roof of the bunker.
\lsnote{During the beam tests the \sout{The}} machine was operated in TBA-like conditions based on simulations 
done to optimize for future TBA runs.
The charge was \SI{30}{nC}, \lsnote{the} buck focusing \lsnote{solenoid} was at \SI{550}{A}, \lsnote{the} matching solenoid was at \SI{200}{A}, 
and quadrupoles 5,6,7 were at \SI{-1.53}{A} and \SI{3.08}{A} in a nearly 1:2:1 configuration.
All accelerating cavities were operated at \SI{-20}{^\circ} off crest to minimize energy spread.
The centroid of the beam was \lsnote{measured \sout{calculated}} for several \lsnote{kicker} voltages, and a linear fit was performed.
The R value for the fit in Figure \ref{fig:linear} is 0.99704. This \lsnote{suggests that} the beam offset 
...
\end{figure}\label{fig:kickerbeamsize}
...
\nrnote{only point to point calculation}
A drift: 
\begin{equation}
R_d = 
\begin{bmatrix}
